...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    FROM
      routes a
    JOIN routes m ON (a.company = m.company AND a.num = m.num)
    JOIN routes b ON (b.stop_id = m.stop_id)
    JOIN routes c ON (c.company = b.company AND c.num = b.num)
    JOIN stops sa ON (a.stop_id = sa.id)
    JOIN stops sm ON (m.stop_id = sm.id)
    JOIN stops sc ON (c.stop_id = sc.id)
    WHERE
      sa.name = 'Craiglockhart'
    AND
      sc.name = 'Sighthill'
