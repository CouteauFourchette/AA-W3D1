...
def all_teachers_join
  ...
  execute(<<-SQL)
    ...
    FROM
      teachers
    LEFT OUTER JOIN depts ON depts.id = dept_id
  SQL
end
...
def all_depts_join
  ...
  execute(<<-SQL)
    ...
    FROM
      teachers
    RIGHT OUTER JOIN depts ON depts.id = dept_id
  SQL
end
...
def teachers_and_divisions
  ...
  execute(<<-SQL)
    SELECT
      name,
      CASE WHEN (dept_id IN (1, 2)) THEN 'Sci' ELSE 'Art' END
    FROM
      teachers
  SQL
end
...
def teachers_and_divisions_two
  ...
  execute(<<-SQL)
    SELECT
      name,
      CASE WHEN dept_id IN (1,2) THEN 'Sci'
